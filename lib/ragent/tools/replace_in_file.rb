...
module Ragent
  module Tools
    class ReplaceInFile
      Result = Struct.new(:path, :patch_file, keyword_init: true) do
        def to_s
      ...
      def call(path, old_text, new_text)
        ...
        return Error.new(message: "old_text appears #{count} times in #{path}; must be unique") if count > 1

        new_content = content.sub(pattern) { new_text.to_s }
        patch_file = generate_diff(path, content, new_content)
        Result.new(path: path, patch_file: patch_file)
      end
      ...
      def generate_diff(path, old_content, new_content)
        ...
        n = Dir[File.join(patches_dir, 'replace_*.patch')].size
        patch_file = File.join(patches_dir, "replace_#{n}.patch")
